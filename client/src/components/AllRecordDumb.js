...
export default function RecordOrderDumb({
  ...
  return (
    <div className="hover:text-white hover:bgItems flex justify-between border text-xl bg-contTable">
      <Link
        className="border w-32 flex justify-center"
        to={`/allorders/${idorder}`}
      >
        <h5 className="flex justify-center text-blue hover:cursor-pointer hover:text-hoverMenu">
          {idorder}
        </h5>
      </Link>
      <h5 className="border w-1/5 flex justify-center">
